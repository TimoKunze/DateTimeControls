...
int GetMonthDays(LPSYSTEMTIME pSystemTime)
{
	SYSTEMTIME systemTime = *pSystemTime;
	systemTime.wDay = 1;
	systemTime.wHour = 0;
	...
	++systemTime.wMonth;
	if(systemTime.wMonth == 13) {
		systemTime.wMonth = 1;
		++systemTime.wYear;
	}
	SystemTimeToFileTime(&systemTime, &fileTime);
	ULARGE_INTEGER nextMonth;
	nextMonth.LowPart = fileTime.dwLowDateTime;
	nextMonth.HighPart = fileTime.dwHighDateTime;
	ULONGLONG diff = nextMonth.QuadPart - timeStamp.QuadPart;
	// diff is in 10^-7 seconds
	ULONGLONG oneDay = 24 * 3600;
	oneDay *= 10000000;
	return static_cast<int>(diff / oneDay);
}